...
describe("doGenerate", () => {
  ...
  it("should extract logprobs", async () => {
    ...
    const response = await provider.completion("openai/gpt-3.5-turbo", { logprobs: 1 }).doGenerate({
      inputFormat: "prompt",
      mode: { type: "regular" },
      prompt: TEST_PROMPT,
    });
    expect(response.logprobs).toStrictEqual(mapHyperbolicCompletionLogProbs(TEST_LOGPROBS));
  });
  ...
  it("should stream text deltas", async () => {
    ...
    expect(await convertReadableStreamToArray(stream)).toStrictEqual([
      ...
      { type: "text-delta", textDelta: "" },
      {
        type: "finish",
        finishReason: "stop",
        logprobs: mapHyperbolicCompletionLogProbs(TEST_LOGPROBS),
